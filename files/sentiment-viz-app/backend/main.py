...
try:
    ...
    MODELS['M1_XGB'] = _load_pkl('M1_XGB')
    MODELS['M2_RF']  = _load_pkl('M2_RF')
    MODELS['M2_XGB'] = _load_pkl('M2_XGB')
    MODELS['M2_LR']  = _load_pkl('M2_LR')
    MODELS['M3']     = _load_pkl('M3')
    META_LR  = _load_pkl('M4_meta_LR')
    META_LGB = _load_pkl('M4_meta_LGB')
    M4_BASE_FULL = _load_pkl('M4_base_models_full')

    bt_path = 'finiteautomata/bertweet-base-sentiment-analysis'
    if bt_path:
        ...
        print(f'  BERTweet remap: {BERTWEET_REMAP}')
    else:
        print('  ⚠️  BERTweet checkpoint not found')
